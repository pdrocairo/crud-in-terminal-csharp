...
namespace crud_in_terminal_csharp.models {
    internal class VendaDAO {
        ...
        public static List<VendaItem> ObterItens(int vendaId) {
            return VendaItemDAO.Listar().Where(i => i.IdVenda == vendaId).ToList();
        }

        public static double CalcularTotal(int vendaId) {
            return ObterItens(vendaId).Sum(i => i.Preco * i.Quantidade);
        }

        public static void FinalizarVenda(int vendaId) {
            Venda? v = VendaDAO.Listar_Id(vendaId);
            if (v != null) {
                v.Total = CalcularTotal(vendaId);
                v.Carrinho = false;
